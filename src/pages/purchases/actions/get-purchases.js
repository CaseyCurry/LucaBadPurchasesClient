...
export default () => {
  return dispatch => {
    ...
    fetch("http://localhost:8086/api/purchases/flagged", {
      headers: {
        // TODO: make dynamic
        Authorization:
        ...
        if (purchaseResponse.status < 200 || purchaseResponse.status > 299) {
          dispatch(
            notifications.addError({
              message: "An error occurred on the server getting the purchases"
            })
          );
          dispatch({
          ...
          fetch("http://localhost:8080/api/commands/transactions/ids", {
            ...
            .then(transactionResponse => {
              if (
                ...
                dispatch(
                  notifications.addError({
                    message:
                      "An error occurred on the server getting the matching transactions"
                  })
                );
                dispatch({
                  type: "GET_PURCHASES_REJECTED"
            ...
            .catch(() => {
              dispatch(
                notifications.addError({
                  message:
                    "An error occurred on the client getting the matching transactions"
                })
              );
              dispatch({
                type: "GET_PURCHASES_REJECTED"
      ...
      .catch(() => {
        dispatch(
          notifications.addError({
            message: "An error occurred on the client getting the purchases"
          })
        );
        dispatch({
          type: "GET_PURCHASES_REJECTED"
