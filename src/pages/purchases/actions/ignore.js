...
export default purchase => {
  return dispatch => {
    ...
    fetch("http://localhost:8086/api/purchases/ignored", {
      method: "POST",
      headers: {
        ...
        if (response.status < 200 || response.status > 299) {
          dispatch(
            notifications.addError({
              message: "An error occurred on the server ignoring the purchase"
            })
          );
          dispatch({
            type: "IGNORE_REJECTED"
          });
        } else {
          dispatch({
            type: "IGNORE_FULFILLED"
          });
        }
      })
      .catch(() => {
        dispatch(
          notifications.addError({
            message: "An error occurred on the client ignoring the purchase"
          })
        );
        dispatch({
          type: "IGNORE_REJECTED"
